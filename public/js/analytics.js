    ...
    <table class="w-full text-xs border-collapse">
      <thead class="bg-gray-50 text-xs uppercase sticky top-0 z-10">
        ...
        <tr class="border-b-2 border-gray-300">
          <th rowspan="2" class="px-2 py-2 border" style="min-width: 40px;">NO.</th>
          <th colspan="8" class="px-2 py-2 border bg-blue-100">발주 정보</th>
          <th colspan="${productionHeaders.length}" class="px-2 py-2 border bg-green-100">생산 공정 (일)</th>
          <th colspan="${shippingHeaders.length}" class="px-2 py-2 border bg-yellow-100">운송 상황 (일)</th>
        ...
        <tr class="border-b-2 border-gray-300">
          <!-- 발주 정보 -->
          <th class="px-2 py-2 border" style="min-width: 60px;">채널</th>
          <th class="px-2 py-2 border" style="min-width: 80px;">생산업체</th>
          <th class="px-2 py-2 border" style="min-width: 100px;">스타일</th>
          <th class="px-2 py-2 border" style="min-width: 50px;">색상</th>
          <th class="px-2 py-2 border" style="min-width: 50px;">사이즈</th>
          <th class="px-2 py-2 border" style="min-width: 60px;">수량</th>
          <th class="px-2 py-2 border" style="min-width: 90px;">발주일</th>
          <th class="px-2 py-2 border" style="min-width: 90px;">입고요구일</th>
          
          <!-- 생산 공정 -->
          ${productionHeaders.map(name => `
            <th class="px-2 py-2 border" style="min-width: 70px;">${name}</th>
          `).join('')}
          ...
          <!-- 최종 현황 -->
          <th class="px-2 py-2 border" style="min-width: 80px;">최종<br>지연일수</th>
...
function renderOrderRow(order, rowNum) {
  ...
  return `
    <tr class="border-b hover:bg-gray-50">
      <td class="px-2 py-2 text-center border">${rowNum}</td>
      ...
      <td class="px-2 py-2 border text-right">${order.qty || 0}</td>
      ...
      <!-- 최종 현황 -->
      <td class="px-2 py-2 border text-center ${finalDelayClass}">${finalDelayDays}</td>
...
function renderProcessCell(order, process, processConfig, category) {
  if (!process) {
    return `<td class="px-2 py-2 border text-center">-</td>`;
  }
  ...
  return `
    <td class="px-2 py-2 border text-center ${cellClass}" ${clickHandler}>
      ${cellContent}
    </td>
  `;
}
